    ...
    def select_top_k_params_by_grad(train_loader, model, criterion, regularizer=None, pgdtype='0', curveflag=False,
        ...
        for param in model.parameters():
            if param.requires_grad and param.grad is not None:
                ...
                grad_norms.append(grad_norm/math.sqrt(param.numel()))
        ...
        if random_select:
            # 随机选择参数
            num_selected = int(len(param_list) * top_k_ratio)
            ...
            # 按梯度范数排序并选择参数
            target_param_count = int(total_params * top_k_ratio)
            sorted_indices = sorted(range(len(grad_norms)), key=lambda i: grad_norms[i], reverse=True)

            selected_params = []
            accumulated_params = 0
            for idx in sorted_indices:
                selected_params.append(param_list[idx])
                accumulated_params += param_sizes[idx]
                if accumulated_params >= target_param_count:
                    break
