...
def msd_v0(model, X,y, epsilon_l_inf = 8/255, epsilon_l_2= 1, epsilon_l_1 = 12,
    ...
    for t in range(num_iter):
        ...
        with torch.no_grad():
            ...
            # # Compare
            # delta_tup = (delta_l_inf,)  # 只包含 delta_l_inf
            #
            # max_loss = torch.zeros(y.shape[0]).to(y.device).half()
            # loss_temp = nn.CrossEntropyLoss(reduction='none')(model(X + delta_l_inf, **kwargs), y)
            #
            # # 更新 delta 和 max_loss
            # delta.data = delta_l_inf.data
            # max_loss = loss_temp.half()
            #
            # # 如果有 max_max_delta 和 max_max_loss 的比较逻辑
            # max_max_delta[max_loss > max_max_loss] = delta.data[max_loss > max_max_loss]
