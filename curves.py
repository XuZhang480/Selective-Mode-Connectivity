...
class CurveParameter(CurveModule):

    def __init__(self, x, fix_points):
        super(CurveParameter, self).__init__(fix_points, ('weight',))
        for i, fixed in enumerate(self.fix_points):
            self.register_parameter(
                'weight_%d' % i,
                Parameter(x, requires_grad=not fixed))

    def forward(self,input,coeffs_t):
        weight_t = self.compute_weights_t(coeffs_t)[0]
        return weight_t
...
#######################################################################
class _LayerNorm(CurveModule):

    def __init__(self, normalized_shape,fix_points, eps=1e-5, affine=True):
        super(_LayerNorm, self).__init__(fix_points, ('weight', 'bias'))
...
def l2_regularizer(weight_decay):
    return lambda model: 0.5 * weight_decay * model.l2 if hasattr(model,'l2') else 0.5 * weight_decay * model.module.l2
